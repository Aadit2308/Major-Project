// ============================================================
//  ENERGY-AWARE GREEDY RELAY — NODE D (DESTINATION) ESP8266
//  VESIT E&TC — Aadit | Co-authors: Samiksha, Anushree
//
//  IDLE = pure RX, nothing transmitted
//  Type message + Enter:
//    1. [INIT] Ready → message typed
//    2. [INIT] Ready to send → send probe HELLO to A
//    3. Wait up to 3s for A's HELLO reply (fresh RSSI)
...
void process_tx_queue() {
  ...
  if (pkt.startsWith("DATA")) {
    ...
    Serial.println("┌─────────────────────────────────────");
    Serial.println("│ [TX] Sent to Node A");
...
void probe_and_send(String dest, String message) {
  ...
  // HELLO format: HELLO|src|battery|hop  (4 fields, 3 separators)
  String hello = "HELLO|" + String(NODE_ID) + "|" + String(current_battery, 1) + "|0";
  LoRa.beginPacket();
  LoRa.print(hello);
  LoRa.endPacket();
  current_battery -= TX_COST;
...
void handle_packet(String raw, int rssi) {
  ...
  // ── HELLO: format HELLO|src|battery|hop ───────────────────
  if (type == "HELLO") {
    String src  = f[1];
    float  batt = f[2].toFloat();   // f[2] = battery  ← FIXED index
    int    hop  = f[3].toInt();     // f[3] = hop       ← FIXED index
    ...
    // Always reply with our own HELLO so the sender can get fresh RSSI
    String reply = "HELLO|" + String(NODE_ID) + "|" + String(current_battery, 1) + "|0";
    LoRa.beginPacket();
    LoRa.print(reply);
    LoRa.endPacket();
    current_battery -= TX_COST;
    ...
    if (waiting_for_reply && src == pending_dest) {
    ...
    if (c == '\n' || c == '\r') {
      ...
      if (serial_buf.length() > 0) {
        if (waiting_for_reply) {
          ...
          Serial.println("│ Step 1: Sending probe HELLO to A...");
          Serial.println("└─────────────────────────────────────");
          probe_and_send("A", serial_buf);
        }
        serial_buf = "";
      }
    } else {
...
void setup() {
  ...
  Serial.println("╔══════════════════════════════════════╗");
  Serial.println("║   LoRa Mesh Node D  (ESP8266)        ║");
  Serial.println("║   IDLE = pure RX, no background TX   ║");
  Serial.println("║   Type message + Enter to send to A  ║");
  Serial.println("╚══════════════════════════════════════╝");
  ...
  Serial.println("[INIT] Idle — type a message and press Enter to send to A");
}
...
void loop() {
  ...
  // 3. Probe timeout — A never replied, use cached table
  if (waiting_for_reply &&
      (millis() - hello_sent_at) >= HELLO_REPLY_WAIT) {
    Serial.println("[PROBE] Timeout — A did not reply, using cached table");
    fire_pending_data();
  }
