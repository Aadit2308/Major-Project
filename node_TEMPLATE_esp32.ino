// ============================================================
//  ENERGY-AWARE GREEDY RELAY — MESH NODE (ESP32 TEMPLATE)
//  VESIT E&TC — Aadit | Co-authors: Samiksha, Anushree
//
//  N-NODE TEMPLATE: flash this SAME file to every ESP32 board
...
//         e.g.  C:sensor reading is 42     -> sends to Node C
//
...
// ─────────────────────────────────────────────────────────
#define NODE_ID  "A"   // Unique single-letter ID: "A","B","C","D","E"...

// ESP32 + SX127x/Ra-02 wiring (VSPI) — same across all ESP32 boards
// LoRa NSS/CS  -> GPIO 5
// LoRa RST     -> GPIO 14
// LoRa DIO0    -> GPIO 26
// LoRa SCK     -> GPIO 18
...
//  Format: <DEST_ID>:<message>   e.g.  "C:hello there"
// ═══════════════════════════════════════════════════════
    ...
    if (c == '\n' || c == '\r') {
      ...
      if (serial_buf.length() > 0) {
        if (waiting_for_reply) {
          ...
          if (sep == -1) {
            Serial.println("[ERR] Format: <DEST_ID>:<message>   e.g. C:hello");
          } else {
...
void setup() {
  ...
  Serial.println("╔══════════════════════════════════════╗");
  Serial.println("║   LoRa Mesh Node " + String(NODE_ID) + "  (ESP32)          ║");
